...
def point2rbox_x(pbox):
    [p1, p2,p3,p4]=torch.chunk(pbox,4,dim=1)
    cx,cy = torch.mean(pbox[...,0::2],dim=1,keepdims=True),torch.mean(pbox[...,1::2],dim=1,keepdims=True)
    angle=torch.atan2((p3[..., 1] - p1[..., 1]), (p3[..., 0] - p1[..., 0])).unsqueeze(1)
    angle=angle- angle.new_tensor(math.pi)/2
    h=torch.sqrt((p1[..., 0] - p3[..., 0]) * (p1[..., 0] - p3[..., 0]) +
                        (p1[..., 1] - p3[..., 1]) * (p1[..., 1] - p3[..., 1])).unsqueeze(1)
    w=torch.sqrt((p2[..., 0] - p4[..., 0]) * (p2[..., 0] - p4[..., 0]) +
                        (p2[..., 1] - p4[..., 1]) * (p2[..., 1] - p4[..., 1])).unsqueeze(1)
    angle = norm_angle(angle, range=[0, 2.0*np.pi])
    return torch.cat((cx,cy,w,h,angle),1)
# def point2rbox(polys):
#     """
#     polys:n*8
...
#     edge2 = torch.sqrt(
#         torch.pow(pt1[..., 0] - pt4[..., 0], 2) + torch.pow(pt1[..., 1] - pt4[..., 1], 2))
#     edge3 = torch.sqrt(
#         torch.pow(pt3[..., 0] - pt4[..., 0], 2) + torch.pow(pt3[..., 1] - pt4[..., 1], 2))
#     edge4 = torch.sqrt(
#         torch.pow(pt2[..., 0] - pt3[..., 0], 2) + torch.pow(pt2[..., 1] - pt3[..., 1], 2))
#     angle=(torch.atan2((pt1[..., 1] - pt4[..., 1]), (pt1[..., 0] - pt4[..., 0]))+torch.atan2((pt2[..., 1] - pt3[..., 1]), (pt2[..., 0] - pt3[..., 0])))/2
#     angle=angle- angle.new_tensor(math.pi)/2
#     angle = norm_angle(angle, range=[0, 2.0*np.pi])
...
#     x_ctr = (pt1[..., 0] + pt3[..., 0]+pt2[..., 0] + pt4[..., 0]) / 4.0
#     y_ctr = (pt1[..., 1] + pt3[..., 1]+pt2[..., 0] + pt4[..., 0]) / 4.0
#     width=(edge1+edge3)/2
#     height=(edge2+edge4)/2
#     # edges = torch.stack([edge1, edge2], dim=1)
#     # width, _ = torch.max(edges, 1)
#     # height, _ = torch.min(edges, 1)

#     return torch.stack([x_ctr, y_ctr, width, height, angle], 1)
...
def point2rbox(polys):
    ...
    angle=torch.atan2((pt1[..., 1] - pt4[..., 1]), (pt1[..., 0] - pt4[..., 0]))
    angle=angle- angle.new_tensor(math.pi)/2
    angle = norm_angle(angle, range=[0, 2.0*np.pi])
    # angles1 = torch.atan2((pt2[..., 1] - pt1[..., 1]), (pt2[..., 0] - pt1[..., 0]))
    # angles2 = torch.atan2((pt4[..., 1] - pt1[..., 1]), (pt4[..., 0] - pt1[..., 0]))
...
def poly_to_rotated_box(polys,labels,refine_angle_with_label=False):#这里转换为np使用cv2的方式获取旋转框
    ...
    rrect:[x_ctr,y_ctr,w,h,angle]
    """
    poly=polys.cpu().numpy().reshape(-1,8)
    outpolys=[]
    ...
    outpolys=polys.new_tensor(outpolys)

    return outpolys#torch.stack([x_ctr, y_ctr, width, height, angles], 1)
# def poly_to_rotated_box(polys):#这里转换为np使用cv2的方式获取旋转框
#     """
#     polys:n*8
#     poly:[x0,y0,x1,y1,x2,y2,x3,y3]
#     to
