...
from mmdet.core import  bbox_areas, force_fp32
from mmdet.core.anchor.guided_anchor_target import calc_region
from mmdet.models.losses import ct_focal_loss
from mmdet.models.utils import (build_norm_layer, bias_init_with_prob)
...
class TTFHead(AnchorHead):
    ...
    def __init__(self,
        ...
        self.wh_planes = 5 if wh_agnostic else 5 * self.num_fg
        self.base_loc = None
    ...
    def forward(self, feats):
        ...
        hm = self.hm(x)
        wh = self.wh(x)
        wh[:,2:4,:,:] = F.relu(wh[:,2:4,:,:] ) * self.wh_offset_base#wh
        return hm, wh
    ...
    def draw_truncate_gaussian(self, heatmap, center, h_radius, w_radius,angle, k=1):
        ...
        gaussian_expand=torch.zeros((max_size,max_size))
    ...
    def target_single_image(self, gt_boxes, gt_labels, feat_shape):
        ...
        """
        gt_rboxes=gt_boxes[...,2:7].clone()
        gt_rboxes[...,0]=(gt_boxes[...,0]+gt_boxes[...,2])/2
        gt_rboxes[...,1]=(gt_boxes[...,1]+gt_boxes[...,3])/2
        ...
        if self.wh_area_process == 'norm':#False
