...
class DotaDataset(CustomDataset):
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        for i, ann in enumerate(ann_info):
            ...
            bbox=np.array(seg).reshape((-1,2))
            xsort=bbox[:,0].argsort()
            bbox_x=bbox[xsort]
            ysort=bbox_x[:,1].argsort()
            [xmin,x2,x3,xmax]=bbox_x[:,0].tolist()
            [ymin,y2,y3,ymax]=bbox_x[ysort][:,1].tolist()
            # ysort+=1
            # ysort=ysort.tolist()
            # myt=int(self.all_types.index(ysort))
            bbox=[xmin,ymin, xmax, ymax,r_w,r_h,angle ,ann['category_id'] ,seg[0], seg[1], seg[2], seg[3], seg[4], seg[5], seg[6], seg[7]]

            # #TODO insert
            # if len(ann['rbox']) > 0:
            #     bbox = ann['segmentation'][0]
            #     #这里是想通过rbox计算point
            #     # rbox=poly_to_rotated_box_single(ann['segmentation'][0]).tolist()
            #     # [cx,cy,r_w,r_h,angle]=rbox#ann['rbox'][0][0:5]
            #     # r0,r1=np.transpose([0,-r_h/2]),np.transpose([r_w/2,0])
            #     # RotateMatrix=np.array([
            #     #                     [np.cos(angle),-np.sin(angle)],
            #     #                     [np.sin(angle),np.cos(angle)]])
            #     # p0=np.transpose(np.dot(RotateMatrix, r0))+[cx,cy]
            #     # p1=np.transpose(np.dot(RotateMatrix, r1))+[cx,cy]
            #     # bbox = [x1, y1, x1 + w , y1 + h,  r_w, r_h, angle,p0[0], p0[1], p1[0], p1[1], self.cat2label[ann['category_id']]]#这里由6改为8#以后以尽量少改动为目标
            #     # bbox = [x1, y1, x1 + w , y1 + h,  r_w, r_h, angle, x1, y1, x1 + w , y1 + h]#这里由6改为8
            # else:#end
        ...
        if gt_bboxes:
            ...
            gt_bboxes = np.zeros((0, 9), dtype=np.float32)#TODO change  gt_bboxes = np.zeros((0, 4), dtype=np.float32)
            gt_labels = np.array([], dtype=np.int64)

        if gt_bboxes_ignore:
            gt_bboxes_ignore = np.array(gt_bboxes_ignore, dtype=np.float32)
        else:
            gt_bboxes_ignore = np.zeros((0, 9), dtype=np.float32)#TODO change 4
